...
struct Admin {
    ...
    std::string password;
    std::string lastLogin;
    std::string role; // "super" or "regular"
};
...
std::string encodeString(const std::string& input) {
    ...
    for (char& c : encoded) {
        c = c + 3; // Caesar cipher with shift 3
    }
    return encoded;
}
...
void logTransaction(const std::string& cardNumber, const std::string& type, 
    ...
    // Find current balance for the account
...
bool calculateBillsRecursive(double amount, int& hundreds, int& fifties, 
    ...
    amount = std::round(amount * 100) / 100; // Round to nearest cent
    ...
    if (choice == 1) {
        ...
        for (auto& account : accounts) {
            if (account.cardNumber == username && 
                ...
                loadingAnimation("Welcome " + account.name);
                return true;
            }
        }
        ...
        for (auto& admin : admins) {
            if (admin.username == username && 
                ...
                loadingAnimation("Welcome " + admin.username);
                return true;
            }
        }
...
void withdraw() {
    ...
    // Check if quick amount selected
    ...
    // Calculate fee
    double fee = calculateFeeRecursive(amount);
    ...
    loadingAnimation("Calculating optimal denomination");
    
    if (!calculateBillsRecursive(amount, hundreds, fifties, twenties, tens, fives, ones)) {
    ...
    if (confirm == 'y' || confirm == 'Y') {
        ...
        // Add notification for low cash alert
...
void changePIN() {
    ...
    // Confirm with security question (simulated)
    std::cout << "\nFor security, please confirm your birth year: ";
    int birthYear;
    std::cin >> birthYear;
    
    // Simple validation - in real system this would be stored
...
void clientMenu() {
    while (currentUser != nullptr) {
        ...
        setColor(GREEN);
        std::cout << std::right << std::setw(30) << "Balance: $" + std::to_string(currentUser->balance);
        resetColor();
        std::cout << " ║\n";
        setColor(CYAN);
        std::cout << "╚══════════════════════════════════════════════════════════════════════╝\n\n";
...
void viewATMCash() {
    ...
    std::cout << "║ $100          | " << std::setw(7) << atmCash.bills100 
              << " | $" << std::setw(25) << std::left << std::to_string(atmCash.bills100 * 100) << "║\n";
    std::cout << "║ $50           | " << std::setw(7) << atmCash.bills50 
              << " | $" << std::setw(25) << (atmCash.bills50 * 50) << "║\n";
    std::cout << "║ $20           | " << std::setw(7) << atmCash.bills20 
              << " | $" << std::setw(25) << (atmCash.bills20 * 20) << "║\n";
    std::cout << "║ $10           | " << std::setw(7) << atmCash.bills10 
              << " | $" << std::setw(25) << (atmCash.bills10 * 10) << "║\n";
    std::cout << "║ $5            | " << std::setw(7) << atmCash.bills5 
              << " | $" << std::setw(25) << (atmCash.bills5 * 5) << "║\n";
    std::cout << "║ $1            | " << std::setw(7) << atmCash.bills1 
              << " | $" << std::setw(25) << (atmCash.bills1 * 1) << "║\n";
    std::cout << "╠════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ TOTAL CASH:   |         | $" << std::setw(25) << std::left << total << "║\n";
    std::cout << "╚════════════════════════════════════════════════════════════╝\n";
...
int main() {
    // Set console title
#ifdef _WIN32
    SetConsoleTitle(TEXT("ATM Banking System"));
#endif
